...
function AdminCalendar(): JSX.Element {
  ...
  const [bookingsData, setBookingsData] = useState<IBookingsData[] | null>(
    null
  );
  const allTimes = [
    "17:00",
      ...
      <div id="calendar-and-input">
        ...
        <div className="calendar-grid">
          ...
          {allTimes.map((time, i) => {
            return (
              <>
                <p className={`time-label time-${time.replace(":", "")}`}>
                  {time}
                </p>
                <div
                  className={`time-line time-${time.replace(":", "")}`}
                ></div>
              </>
            );
          })}
          {bookingsData &&
            bookingsData.map((booking, i) => {
              return <OneBookingCalendarDisplay key={i} booking={booking} />;
            })}
        </div>
      </div>
    </>
